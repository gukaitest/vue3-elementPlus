...
网络断开后，正在进行的请求会失败（超时、net::ERR_NETWORK 等）。要支持“断开后继续上传”，需要做三件事：**检测失败原因**、**把未成功的切片归队**、**网络恢复后继续上传**（自动或手动）。

#### 8.1 请求失败时的处理（归队 + 状态）

单个切片上传使用 `uploadFile`（内部带 `AbortController`）。请求失败时（网络断开、超时、5xx 等），应在 catch 或错误分支中：**把该切片从“正在上传”放回“待上传”**，并可选地标记为“因网络中断”以便自动重试。
...
const uploadChunk = async (needObj: FileChunk) => {
  try {
    ...
    if (!res || res.response?.data?.code !== '0000') {
      ...
      return uploadChunk(needObj); // 重试
    }

    // 成功：从 whileRequests 移除、更新 finishNumber、继续下一批...
  } catch (error: any) {
    // 网络断开、超时等会进入这里
    ...
    // 从“正在上传”移回“待上传”，便于继续上传时再次尝试
    taskArrItem.whileRequests = taskArrItem.whileRequests.filter(
      item => item.chunkHash !== needObj.chunkHash
    );
    taskArrItem.allChunkList.push(needObj);

    if (isNetworkError) {
      taskArrItem.state = 5; // 上传中断（可由网络断开导致）
    }
    taskArrItem.errNumber += 1;
    if (taskArrItem.errNumber <= 3) {
      return uploadChunk(needObj); // 重试
...
- **失败即归队**：`whileRequests` 中移除该切片，并 push 回 `allChunkList`，这样无论是“继续上传”还是“网络恢复后重试”，都只会上传未完成的切片。
- **状态 5（上传中断）**：可用于区分“用户暂停”和“网络断开”，便于 UI 展示“网络异常，请检查网络后继续”。

#### 8.2 网络恢复后继续上传（两种方式）

**方式一：用户点击“继续上传”**

文档已有的 `resumeUpload` 逻辑不变：把当前 `whileRequests` 全部放回 `allChunkList`，清空 `whileRequests`，再调用 `uploadSignleFile`。这样未完成的切片（包括因网络断开失败的）都会重新上传。
...
网络恢复时自动对“因网络中断”的任务执行继续上传：

```typescript
// 在应用初始化或上传模块加载时注册
const handleOnline = () => {
  uploadFileList.value.forEach((taskArrItem: FileUploadStatus) => {
    // 仅对“上传中断”状态的任务自动继续
    if (taskArrItem.state === 5 && taskArrItem.whileRequests.length + taskArrItem.allChunkList.length > 0) {
      taskArrItem.state = 2;
      taskArrItem.errNumber = 0; // 可选：重置错误计数
      taskArrItem.allChunkList.push(...taskArrItem.whileRequests);
      taskArrItem.whileRequests = [];
      uploadSignleFile(taskArrItem);
    }
...
// 组件卸载时：window.removeEventListener('online', handleOnline);
```

这样在断网期间失败的切片会被归队，恢复网络后要么用户点“继续”，要么由 `online` 事件自动继续，都只会上传未完成的切片，实现“网络断开后继续上传”。

#### 8.3 与断点续传的关系

- **断点续传**：每次“开始/继续上传”前调用 `checkFile`，用服务器返回的 `uploadedList` 过滤掉已成功落盘的切片，只上传 `allChunkList` 里剩余部分。
- **网络断开后继续**：失败切片已归队到 `allChunkList`，下次 `uploadSignleFile` 时依然只会发这些未完成切片；若中途刷新页面，再次选择同一文件时会重新走 `checkFile`，同样只传未完成的切片。

因此“网络断开后继续上传”与断点续传是兼容的：都是“只传未完成的切片”，区别在于是否刷新页面、是否重新校验 `uploadedList`。

#### 8.4 小结

| 环节           | 做法 |
|----------------|------|
| 请求失败时     | 将该切片从 `whileRequests` 移回 `allChunkList`，必要时置 state=5 |
| 用户继续       | `resumeUpload`：归队 `whileRequests` 后调用 `uploadSignleFile` |
| 网络恢复自动继续 | 监听 `online`，对 state=5 且仍有未完成切片的任务执行上述继续逻辑 |
| 与断点续传     | 继续时只上传 `allChunkList`；刷新后重新 `checkFile` 过滤已上传切片 |
...
### Q13: 网络断开连接后，如何实现继续上传？

**答案**：

**核心思路**：请求失败时把未成功的切片从“正在上传”放回“待上传”，网络恢复后（或用户点击继续）只上传这些未完成切片，配合断点续传接口过滤已落盘切片。

**实现要点**：

1. **失败时归队**：在 `uploadChunk` 的 catch 或错误分支中，将当前切片从 `whileRequests` 移除并 push 回 `allChunkList`，这样该切片会在下次“继续上传”时再次被上传。
2. **区分网络错误**：通过 `error?.message === 'Network Error'`、`error?.code === 'ERR_NETWORK'` 等判断为网络问题，可将任务状态设为“上传中断”（如 state=5），便于展示“网络异常，请检查后继续”。
3. **用户手动继续**：提供“继续上传”按钮，调用 `resumeUpload`：把当前 `whileRequests` 全部放回 `allChunkList`，再调用 `uploadSignleFile`，只会上传未完成切片。
4. **网络恢复自动继续**：监听 `window` 的 `online` 事件，对状态为“上传中断”且仍有未完成切片的任务，执行与“继续上传”相同的逻辑（归队 `whileRequests` 后调用 `uploadSignleFile`）。
5. **与断点续传一致**：继续时只上传 `allChunkList` 中的切片；若用户刷新页面后再次选择同一文件，会重新走验证接口拿到 `uploadedList`，过滤掉已上传切片，实现断网后的断点续传。
...
taskArrItem.allChunkList.push(needObj);
if (isNetworkError) taskArrItem.state = 5;

// 网络恢复后自动继续
window.addEventListener('online', () => {
  uploadFileList.value.forEach(item => {
    if (item.state === 5 && (item.whileRequests.length + item.allChunkList.length) > 0) {
      item.allChunkList.push(...item.whileRequests);
      item.whileRequests = [];
      uploadSignleFile(item);
    }
  });
});
```
