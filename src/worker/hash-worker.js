...
async function calculateChunksHash(fileChunkList) {
  ...
  async function loadNext(index) {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = e => {
        ...
        globalThis.postMessage({ percentage }); // 发送进度到主线程
  ...
  try {
    ...
    globalThis.postMessage({ percentage: 100, fileHash, fileChunkList }); // 发送最终结果到主线程
    globalThis.close(); // 关闭Worker
  } catch (err) {
    globalThis.postMessage({ name: 'error', data: err }); // 发送错误到主线程
    globalThis.close(); // 关闭Worker
  }
}
